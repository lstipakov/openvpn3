...
* :code:`class TunBuilderBase` in `<openvpn/tun/builder/base.hpp>`_ â€”
...
* :code:`class ExternalPKIBase` in `<openvpn/pki/epkibase.hpp>`_ â€”
...
* :code:`class LogReceiver` in `<client/ovpncli.hpp>`_ â€”
...
1. :code:`class ClientConnect` in `<openvpn/client/cliconnect.hpp>`_ â€”
   The top-layer object in an OpenVPN client connection.
2. :code:`class ClientProto::Session` in `<openvpn/client/cliproto.hpp>`_ â€”
   The OpenVPN client protocol object that subinstantiates the transport
   and tun layer objects.
3. :code:`class ProtoContext` in `<openvpn/ssl/proto.hpp>`_ â€”
   The core OpenVPN protocol implementation that is common to both
   client and server.
4. :code:`class ProtoStackBase<Packet>` in `<openvpn/ssl/protostack.hpp>`_ â€”
...
* **UDP** â€” `<openvpn/transport/client/udpcli.hpp>`_
* **TCP** â€” `<openvpn/transport/client/tcpcli.hpp>`_
* **HTTP Proxy** â€” `<openvpn/transport/client/httpcli.hpp>`_
...
2. Use an OS-specific model such as:
   ...
   * **Linux** â€” `<openvpn/tun/linux/client/tuncli.hpp>`_
   * **Windows** â€” `<openvpn/tun/win/client/tuncli.hpp>`_
   * **Mac OS X** â€” `<openvpn/tun/mac/client/tuncli.hpp>`_
